...
function get_sets()
    ...
    HerculeanFeet.TA = { name="Herculean Boots", augments={'Accuracy+19 Attack+19','"Triple Atk."+2','VIT+9','Attack+14',}}
    HerculeanFeet.DT = { name = "Herculean Boots", augments = { 'Accuracy+23', 'Damage taken -3%' } }
    HerculeanFeet.Idle = { name="Herculean Boots", augments={'MND+4','Accuracy+16 Attack+16','"Refresh"+2',}}
...
--]]	
    ...
    sets.JA.Efflux = { legs = "Hashishin Tayt", back= Rosmerta.WSD }
...
send_command('bind !q input /equip ring2 "Dim. Ring (Dem)"; input /echo Reisenjima; wait 12; input /item "Dim. Ring (Dem)" <me>;')
...
function mc_Magic(spell, act)
    ...
    if spell.skill == 'Blue Magic' then
		...
		if buffactive['Diffusion'] then --Putting Diffusion <wait 1> before Unbridled Learning <wait 1> (instead of the other way around) in your macro will decrease issues
			ChangeGear({feet="Mirage Charuqs +2"})
			msg("Mirage Charuqs equiped !!!!")
		end
    end
        ...
        if MagicalSpells:contains(spell.english) then
            ...
            elseif BlueMagic_Light:contains(spell.english) then
				if spell.english == 'Magic Hammer' then
					...
					ChangeGear(set_combine(sets.BlueMagic.INT[sets.BlueMagic.INT.index[MAB_ind]],{
						ring2 = "Weatherspoon Ring" ,waist = 'Hachirin-no-Obi'}))
				end
			end
		end
