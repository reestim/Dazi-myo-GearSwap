...
--]]	
    ...
    sets.str.index = { 'AttackUncap', 'AttackCap', 'Accuracy' }
    Expiacion_ind = 1

    sets.str.AttackUncap = {
        ammo="Ginsen",
	...
	sets.str.AttackCap = set_combine(sets.str.AttackUncap, {
		waist = "Fotia Belt",
		})
    ...
    Judgment_ind = 1
    sets.Judgment.Attack = set_combine(sets.str.Attack, {})

    sets.Judgment.Accuracy = set_combine(sets.str.Accuracy, {})
	...
	sets.BlueMagic.MagicAccuracyFull = {
        main="",
		sub= "",
		ammo="Ginsen",
		head="Jhakri Coronal +1",
...
--  	Empty Set, nothing to put here yet
    }
    ...
    sets.Enhancing.ProShell = set_combine(sets.BlueMagic.Buffs, {ring2 = "Sheltered Ring"})
...
function buff_change(n, gain, buff_table)
    ...
    if S{"terror","petrification","sleep","stun"}:contains(name) then
        if gain then
			...
			send_command('@input /p HELP I HAVE ')
...
function pc_JA(spell, act)
	...
	STRWS = S { "Expiacion", "Circle Blade", "Savage Blade", "True Strike", "Judgment" }
	
	if spell.type == 'WeaponSkill' then
		if spell.english == 'Requiescat' then
            ChangeGear(sets.Requiescat[sets.Requiescat.index[Requiescat_ind]])
		elseif STRWS.contains(spell.english)  then
            ChangeGear(sets.str[sets.str.index[str_ind]])
